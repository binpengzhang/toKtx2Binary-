...
public:
	...
	int BufferToKtx2(FILE* intFile, unsigned char** outBuffer, size_t& outSize);
	virtual void usage();
...
toktxApp theApp;
#include<stdio.h>
int toktxApp::BufferToKtx2(FILE* intFile, unsigned char** outBuffer, size_t& outSize)
{
	if (!intFile)
		return 1;
	KTX_error_code ret;
	...
	//validateOptions();
	options.etc1s = 1;//开启压缩
	options.ktx2 = 1;//开启ktx2
  ...
  //       it < options.infiles.end();
   //      it++, i++)
	...
	//{
		...
		/*ktx_uint8_t* buffer;
		ktx_size_t size1;*/
		ret = ktxTexture_WriteToMemory(ktxTexture(texture), &(*outBuffer), &outSize);
...
int toktxApp::main(int argc, _TCHAR* argv[])
{
	KTX_error_code ret;
	ktxTextureCreateInfo createInfo;
	ktxTexture* texture = 0;
	...
	if (f) {
		...
		//ktx_uint8_t* buffer;
		//ktx_size_t size1;
		//ktxTexture_WriteToMemory(ktxTexture(texture), &buffer,&size1);
		////将buffer写入文件
		//fwrite(buffer,1,size1,f);
...
{
	toktxApp theApp2;
	int ret = theApp2.main(argc, argv);
	return ret;
}
...
{
	toktxApp theApp2;
	int ret = theApp2.BufferToKtx2(file, outBuffer, outSize);
	return ret;
}